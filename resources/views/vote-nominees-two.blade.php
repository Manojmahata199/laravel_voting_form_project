        ...
        <div class="container-fluid bg-gray" id="accordion-style-1">
            <div class="container">
                <section>
                    <div class="row">
                        <div class="col-12">
                            <h1 class="text-green mb-4 text-center">Vote For Favorite Nominee</h1>
                        </div>
                                    ...
                                    <div class="card-header" id="headingOne">
                                        <h5 class="mb-0">
                                            <button class="btn btn-link btn-block text-left" type="button" data-toggle="collapse" data-target="#collapse{{$key}}" aria-expanded="true" aria-controls="collapse{{$key}}">
                                              <i class="fa fa-angle-double-right mr-3"></i>{{$category->name}}
                                            </button>
                                        </h5>
                                    </div>
                                    <div class="d-none">{{ $show = ($key === 0) ? 'show' : '' }}</div>
                                    <div id="collapse{{$key}}" class="collapse {{$show}} fade" aria-labelledby="headingOne" data-parent="#accordionExample">
                                        @foreach ($subcategories as $key => $subcategory)  
                                        @php if (count($participants[$category->id][$subcategory->id]) > 0): @endphp  
                                        <div class="card-body" >
                                            <div class="container event-schedule-area-two">
                                                <h5 class="mt-2 bg-white rounded shadow p-2 bg-white mx-auto d-block">{{$subcategory->name}}</h5>
